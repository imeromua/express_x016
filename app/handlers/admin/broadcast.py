...
async def start_broadcast(message: Message, state: FSMContext = None) -> None:
    """Виклик з reply-кнопки або inline callback."""
    if state:
...
async def receive_broadcast_content(message: Message, state: FSMContext) -> None:
    ...
    await message.answer("Вище буде повідомлення для розсилки:")
...
@router.callback_query(F.data == "broadcast:confirm", AdminStates.waiting_broadcast_confirm)
async def cb_broadcast_confirm(
    ...
    for i, uid in enumerate(user_ids, 1):
        try:
            await bot.forward_message(chat_id=uid, from_chat_id=src_chat_id, message_id=src_message_id)
            sent += 1
